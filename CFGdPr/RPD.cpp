...
Input
The first line of input contains a single integer t (1≤t≤10000) denoting the number of test cases. t test cases follow.

The first line of each test case contains two integers n (1≤n≤2⋅105) and k (k=2).
...
For each query, print a single integer y (0≤y≤2⋅107). Let the current password be x. After that, read an integer r.

If x=y, you will read r=1 and the test case is solved. You must then continue solving the remaining test cases.

Else, you will read r=0. At this moment the password is changed to a number z such that x⊕kz=y.
...
The first and only line of each test case should contain two integers n (1≤n≤2⋅105) and k (k=2) denoting the number of queries and the base respectively. The optimal original password is automatically decided by the adaptive interactor.

You must ensure that the sum of n over all test cases does not exceed 2⋅105.
...
The first query is 3. It is not equal to the current password. So, 0 is returned, and the password is changed to 1 since 2⊕21=3.

The second query is 4. It is not equal to the current password. So, 0 is returned, and the password is changed to 5 since 1⊕25=4.
